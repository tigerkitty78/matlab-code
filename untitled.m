...
folderPath = 'C:\Users\dasantha\Desktop\voice.csv';  % this is the folder the data files are in
%files = dir(fullfile(folderPath, 'voice.csv'));  % Get all .csv files

fullFileName = fullfile(folderPath, 'voice.csv');  % Create the full path

data = readtable(fullFileName);  % Read the file using the full path


if exist(fullFileName, 'file')
    data = readtable(fullFileName);  % Read the file
else
    error('File not found: %s', fullFileName);  % Show error if file is missing
end
...
'Q25' 'Q75' 'IQR' 'skew' 'kurt' 'sp_ent' 'sfm' 'mode' 'centroid' 'meanfun' ...
...
Testing_Data = array2table(Testing_Data, 'VariableNames', {'meanfreq' 'sd' ...
'median' 'Q25' 'Q75' 'IQR' 'skew' 'kurt' 'sp_ent' 'sfm' 'mode' 'centroid' ...
